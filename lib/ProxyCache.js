...
var ProxyCache = function(options){
    ...
    proxyCache.rejectList = options.rejectList || [];
    proxyCache.acceptList = options.acceptList || [];
...
ProxyCache.prototype.createMiddleware = function() {
    ...
    var listChecking = function (list, req, urlPathList) {
        if (list) {
            return list.some(function (pattern, i){
                // ignore list elements can be either a regex or an string for exact match
                if (pattern && pattern instanceof RegExp){
                    return pattern.test(req.url);
                } else if (pattern && typeof pattern === 'string') {
                    if (urlPathList.indexOf(pattern) > 0) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    return function proxyCacheMiddleware(req, res, next) {
        var urlPathList = req.url.split('/');
        console.log('ip',req.headers['x-forwarded-for'], 'are requesting', req.url);
        var isRoot = (urlPathList.length === 2 );
        console.log(isRoot);
        var shouldReject = listChecking(proxyCache.rejectList, req);
        var shouldAccept = listChecking(proxyCache.acceptList, req, urlPathList);
        if (isRoot || shouldAccept) {
            if (shouldReject) {
                res.send(404);
            } else {
